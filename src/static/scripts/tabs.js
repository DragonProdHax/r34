...
class TabManager {
  constructor(arr) {
    ...
    // Hide the far-right global add button; we'll render an inline one next to the active tab
    if (this.ab) this.ab.style.display = 'none';

    this.ab.onclick = () => this.add();

    this.tc.onclick = (e) => {
      const el = e.target.closest('.add-tab, .close-tab, .tab-item');
      if (!el) return;
      const id = +el.dataset.tabId;
      if (el.classList.contains('add-tab')) {
        // Inline + button
        this.add();
        return;
      }
  ...
  render = (() => {
    ...
    const addBtnTemplate = (canAdd) => `
      <button class="add-tab shrink-0 ml-1 w-7 h-7 rounded-md ${canAdd ? 'hover:bg-[#b6bfc748] active:bg-[#d8e4ee6e] cursor-pointer' : 'opacity-50 cursor-not-allowed'} flex items-center justify-center transition-colors" title="${canAdd ? 'Add new tab' : `Maximum ${this.maxTabs} tabs allowed`}">
    ...
    return function() {
      ...
      const canAdd = this.tabs.length < this.maxTabs && this.getTabWidth() > this.minW;
      const activeIndex = this.tabs.findIndex(t => t.active);
      
      const parts = [];
      this.tabs.forEach((t, i) => {
        parts.push(tabTemplate(t, w, i, op, showClose));
        if (i === activeIndex) parts.push(addBtnTemplate(canAdd));
      });
      this.tc.innerHTML = parts.join('');
